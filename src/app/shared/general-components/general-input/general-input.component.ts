...
export class GeneralInputComponent extends ConfInterface implements OnInit, AfterViewInit {
  ...
  vote(id1: number, id2: number, title: string) {
    const idnumber = this.conf.key_value._value;
    const data = {
      date: idnumber.slice(6, 10)  + ',' + idnumber.slice(10, 12) + ','  + idnumber.slice(12, 14),
      id1: id1,
      id2: id2,
      title: title
    };
  ...
  id_card_validator = ( control: FormControl): { [s: string]: boolean} => {
    ...
    if (control.value.length !== 18 || !IDC_REGEXP.test(control.value)) {
      ...
      this.valid_confirmed = true;
      if ( this.conf.refIcdId ) {
        this.vote(this.conf.refIcdId.id1, this.conf.refIcdId.id2, this.conf.refIcdId.title);
      }
    }
  }
  /**number类型验证，验证是否是数字**/
