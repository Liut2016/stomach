...
import { FollowDiabetesList} from '@app/follow/follow-up-diabetes/followDiabetesList';
...
export class DialogOverviewFollowDiabetesDialogComponent {
  constructor(
    public dialogRef: MatDialogRef<DialogOverviewFollowDiabetesDialogComponent>,
...
@Component({
  selector: 'app-follow-up-diabetes',
  templateUrl: './follow-up-diabetes.component.html',
  styleUrls: ['./follow-up-diabetes.component.css']
})
export class FollowUpDiabetesComponent implements OnInit {
  current = 0;
  followList;
  unconfirmedList = [];
  PID;
  ...
  ngOnInit() {
    ...
    if (this.routeParams.length === 1) {
      ...
      this.followList = new FollowDiabetesList().items;
  ...
  openDialog(): void {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (!result) { this.router.navigate(['survey/diabetesFollowSingleview/' + this.PID]); }
    });
  }
  validateAllPage() {
